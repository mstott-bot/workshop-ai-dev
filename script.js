...
if($("saveTargets")) $("saveTargets").addEventListener("click",()=>{const draft=targetsDraft(),mixTotal=draft.dailyRetailPct+draft.dailyWarrantyPct+draft.dailyInternalPct;if(Math.abs(mixTotal-100)>0.01){alert(`Daily Diary Work Mix must total 100%. Current total: ${mixTotal.toFixed(0)}%.`);return}saveCurrentTargetsSnapshot();targets={...targets,...draft};window.targets=targets;addRateHistoryEntry(targets.rateEffectiveDate,targets.retailRate,targets.warrantyRate,targets.internalRate);saveTargetsStore();renderKpiTargetsPreview();render();window.renderWorkshopDiary?.();alert("Workshop settings updated successfully")});
