...
function render(){populateTechnicianSelects();renderManager();renderServicePartsAlert();renderTechnicianPartsAlert();renderCompletedReportsInbox();renderTech();renderDash();renderTechnicianSetup();renderReportsInterface();renderDailyPlanner();renderPartsQueues();renderPartsManagement();renderFutureBookings();renderEndOfDayBriefing();loadTargetsInputs();refreshActiveJobPartsPanel()}function isFinishedForDailyCleanup(j){const s=(j.status||"").toLowerCase();return !!j.completedAt||s.includes("complete")||s.includes("completed")||s.includes("ready for collection")||s.includes("collected")||s.includes("closed")}function isLiveWorkshopJob(j){const today=todayISO();const jobDate=String(j.bookingDate||today).slice(0,10);if(jobDate===today)return true;if(jobDate<today&&!isFinishedForDailyCleanup(j))return true;return false}function getLiveWorkshopJobs(){return jobs.filter(isLiveWorkshopJob)}function renderManager(){const liveJobs=getLiveWorkshopJobs();$("managerJobs").innerHTML=liveJobs.length?liveJobs.map(j=>card(j,true,true)).join(""):"No live jobs for today."}
...
function todayOperationalJobs(){
  const today=todayISO();
  return jobs.filter(j=>((j.bookingDate||today)===today) || !completed(j));
}
...
function renderDash(){
  ...
  if($("ownerStats")) $("ownerStats").innerHTML=`<div class="stat"><strong>${jobs.length}</strong>Jobs</div><div class="stat"><strong>${m.available.toFixed(1)}</strong>Available Hrs</div><div class="stat"><strong>${m.sold.toFixed(1)}</strong>Sold Hrs</div><div class="stat"><strong>${pctText(m.productivity)}</strong>Productivity</div><div class="stat"><strong>${pctText(m.workshopEff)}</strong>Efficiency</div><div class="stat ${m.carried.length>0?'warn':'good'}"><strong>${m.carried.length}</strong>Carry-over</div>`;
