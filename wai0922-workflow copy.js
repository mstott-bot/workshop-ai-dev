...
(function(){
  ...
  function installManagerOrderingCards(){
    ...
    parts.innerHTML=`<h2>📦 Parts Ordering</h2><p class="muted">Only parts that still require Service Manager action are shown here. Received parts move through the existing Parts workflow.</p>
      <div class="stats"><div class="stat ${need.length?'bad':'good'}"><strong>${need.length}</strong>Need Ordering</div><div class="stat ${awaiting.length?'warn':'good'}"><strong>${awaiting.length}</strong>Awaiting Delivery</div></div>
      <div class="job-list">${[...need,...awaiting].length?[...need,...awaiting].map(({job,item,type})=>{const status=itemStatus(item,type);return `<div class="parts-alert-item"><strong>📦 PART — ${esc(job.reg||'No registration')} — ${esc(job.technician||'Unassigned')}</strong><br><span>${esc(label({item,type}))}</span><br><span><strong>Status:</strong> ${esc(status)}</span><div class="parts-actions wai0922-actions">${status==='Requested'?`<button onclick="openPartsOrderForm('${esc(job.id)}','${esc(item.id)}')">Order Parts</button>`:actionButtons({job,item,type},'Service Manager')}<button onclick="openJob('${esc(job.id)}')">Open Job</button><button onclick="showTimelineModal('${esc(job.id)}')">Timeline</button></div></div>`}).join(''):`<div class='job-card good'><p>✅ No parts orders require Service Manager action.</p></div>`}</div>`;
  }
