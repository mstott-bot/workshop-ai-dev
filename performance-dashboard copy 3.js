(function () {
  ...
  function renderPerformanceDashboard() {
    ...
    const productivity=actualHours>0?(labourHours/actualHours)*100:0;
    const utilisation=Number(targets.availableHours||0)>0?(actualHours/Number(targets.availableHours))*100:0;
    stats.innerHTML=`
    ...
    coach.innerHTML=`<div class="coach-card ${recovery>=Number(targets.labourRecovery||90)?"good":recovery>=80?"warn":"bad"}">
      ...
      <p>Productivity is ${productivity.toFixed(0)}% from sold hours ÷ actual job-clocked hours; utilisation is ${utilisation.toFixed(0)}% from clocked hours ÷ available hours.</p>
