...
function revenuePanel(r){
  ...
  const opGp=v.workInProgress.gp+v.readyToInvoice.gp;
...
window.addEventListener('wai-finance-updated',()=>render());document.addEventListener('storage',e=>{if(e.key==='wai0991Invoices'||e.key==='wai0991FinanceSettings')render();});document.addEventListener('DOMContentLoaded',()=>{inject();render();setInterval(render,60000);});setTimeout(()=>{inject();render();},500);
})();
